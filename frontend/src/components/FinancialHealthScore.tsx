...
import { TrendingUp, Shield, PiggyBank, AlertCircle, Sparkles, ChevronRight } from 'lucide-react';
...
function CircularProgress({ data, animate }: { data: ScoreData; animate: boolean }) {
  const radius = 52;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative w-32 h-32">
        ...
        >
          {/* Background circle */}
          <circle cx="60" cy="60" r={radius} fill="none" stroke="#EAF4FF" strokeWidth="10" />
          {/* Progress circle */}
          <circle
            cx="60"
            cy="60"
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-bold font-display text-deep-navy">{displayScore}</span>
          <span className="text-xs text-deep-navy/50">/ {data.maxScore}</span>
        </div>
      </div>
      <div className="text-center">
        <div className="flex items-center justify-center gap-1.5 mb-1">
          <span style={{ color: data.strokeColor }}>{data.icon}</span>
          <span className="text-sm font-semibold text-deep-navy">{data.label}</span>
        </div>
        <p className="text-xs text-deep-navy/60 max-w-[120px]">{data.tip}</p>
      </div>
    </div>
  );
}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <MotionReveal>
          <div className="text-center mb-12">
            ...
            <p className="text-deep-navy/60 mt-6 max-w-xl mx-auto">
              A holistic view of your financial wellness across four key dimensions.
            </p>
          </div>
        </MotionReveal>
        ...
        <div ref={ref} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <MotionReveal delay={100} className="lg:col-span-2">
            <div className="glass-card rounded-2xl p-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                {scores.map((score) => (
                  <CircularProgress key={score.label} data={score} animate={isInView} />
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-gold/10">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-sm text-deep-navy/60">Overall Health Score</span>
                    <div className="text-2xl font-bold font-display text-deep-navy">
                      64.5{' '}
                      <span className="text-sm font-normal text-deep-navy/50">/ 100</span>
                    </div>
                  </div>
          ...
          <MotionReveal delay={200}>
            <div className="glass-card-navy rounded-2xl p-6 h-full">
              ...
              <div className="space-y-4">
                {aiRecommendations.map((rec, i) => (
                  <div
                    key={i}
                    className="bg-white/10 rounded-xl p-3 hover:bg-white/15 transition-colors cursor-pointer group"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${rec.priorityColor}`}
                          >
                            {rec.priority}
                          </span>
                        </div>
                        <p className="text-xs text-white/60 leading-relaxed">{rec.desc}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 mt-2 text-gold text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity">
