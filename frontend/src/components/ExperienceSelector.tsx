...
import { Leaf, Users, GraduationCap, Briefcase, Check, ArrowRight } from 'lucide-react';
import { useExperience, ExperienceType } from '../contexts/ExperienceContext';
...
const experiences: ExperienceOption[] = [
  {
    ...
    features: ['Kisan Credit Card', 'Agri Loans', 'Village ATMs', 'Vernacular Support'],
  ...
  },
  {
    ...
    features: ['Higher FD Rates', 'Doorstep Banking', 'Pension Services', 'Health Insurance'],
  ...
  },
  {
    ...
    tagline: 'Start Smart',
    features: ['Zero Balance Account', 'Education Loans', 'Student Credit Card', 'Scholarships'],
  ...
  },
  {
    ...
    features: ['Current Accounts', 'Business Loans', 'Trade Finance', 'Payroll Services'],
    ...
    <section className="py-20 lg:py-28" style={{ backgroundColor: 'oklch(97% 0.015 270)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <MotionReveal className="text-center mb-14">
          ...
          >
            Choose your experience and discover banking solutions tailored specifically for you.
          </p>
        </MotionReveal>
              ...
              <MotionReveal key={String(exp.id)} delay={index * 80}>
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isSelected ? 'var(--shadow-lavender)' : 'var(--shadow-soft)',
                    transition: prefersReducedMotion ? 'none' : 'all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)',
                    transform: isSelected ? 'translateY(-4px)' : 'translateY(0)',
                  }}
                  onMouseEnter={(e) => {
                    if (!isSelected && !prefersReducedMotion) {
                      (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(-4px)';
                      (e.currentTarget as HTMLButtonElement).style.boxShadow = 'var(--shadow-lavender)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isSelected) {
                    ...
                    {exp.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-2 text-xs">
                        <Check
                          ...
                          style={{ color: isSelected ? exp.color : 'var(--muted-foreground)' }}
                        />
                        <span style={{ color: isSelected ? 'var(--foreground)' : 'var(--muted-foreground)' }}>
                          {feature}
                        </span>
                      </li>
                    ))}
