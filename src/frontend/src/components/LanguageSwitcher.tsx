...
export default function LanguageSwitcher() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="w-44 bg-white/95 backdrop-blur-md border-lavender-200/60 shadow-soft-md"
      >
        {languages.map((lang) => (
          <DropdownMenuItem
            key={lang.code}
            onClick={() =>
              setLanguage(lang.code as "en" | "hi" | "mr" | "ta" | "te" | "bn")
            }
            className={`flex items-center justify-between cursor-pointer text-[#1A1A1A] hover:text-[#000000] ${
              language === lang.code
                ? "bg-lavender-50 font-semibold text-[#000000]"
                : ""
            }`}
          >
