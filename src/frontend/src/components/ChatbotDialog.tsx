...
export default function ChatbotDialog() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally scroll on message/typing changes
  useEffect(() => {
  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (
      ...
      (lowerMessage.includes("dsouza") || lowerMessage.includes("bank"))
    ) {
    ...
    if (
      ...
      (lowerMessage.includes("based") || lowerMessage.includes("stage"))
    ) {
    ...
    if (lowerMessage.includes("first job") || lowerMessage.includes("career")) {
    ...
    if (lowerMessage.includes("family") || lowerMessage.includes("planning")) {
        ...
        <div className="space-y-4">
          ...
          {isSpeaking && (
            <div className="flex items-center gap-2 text-sm text-[#000000]">
              <Volume2
                className={`h-4 w-4 ${prefersReducedMotion ? "" : "animate-pulse"}`}
              />
