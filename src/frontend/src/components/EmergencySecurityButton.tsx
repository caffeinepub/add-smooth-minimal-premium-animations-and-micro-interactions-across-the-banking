...
const actions = [
  {
    ...
    desc: "Instantly block all debit and credit cards",
    icon: <CreditCard className="w-4 h-4" />,
  },
  {
    id: "disable-upi",
    label: "Disable UPI",
    desc: "Stop all UPI transactions immediately",
    icon: <Smartphone className="w-4 h-4" />,
  },
  {
    id: "alert-support",
    label: "Alert Support Team",
    desc: "Notify our 24/7 security team",
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#333333]">
              Select the security actions you want to activate immediately. This
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span
                      className={
                        checked[action.id] ? "text-red-600" : "text-[#555555]"
                      }
                    >
                      {action.icon}
                    </span>
                    <Label
                      htmlFor={action.id}
                      className="font-semibold text-[#000000] cursor-pointer"
                    >
                      {action.label}
                    </Label>
                  </div>
                  <p className="text-xs text-[#333333] mt-0.5">{action.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <DialogFooter className="gap-2">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl border-2 border-[#cccccc] text-[#000000] font-semibold hover:bg-[#f5f5f5] transition-colors"
            >
              Cancel
            </button>
            <button
