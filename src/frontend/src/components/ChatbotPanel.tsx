...
const RESPONSES: Record<ChatLang, Record<string, string>> = {
  en: {
    default:
      "Thank you for your message. Our team will assist you shortly. For urgent matters, please call our 24/7 helpline.",
    balance:
      "To check your account balance, please log in to your DSOUZA BANK account or visit the nearest branch.",
    loan: "We offer home loans, personal loans, and business loans at competitive rates. Would you like to know more?",
    transfer:
      "You can transfer funds via NEFT, RTGS, or IMPS through our secure online banking portal.",
  ...
  hi: {
    ...
    transfer:
      "आप हमारे सुरक्षित ऑनलाइन बैंकिंग पोर्टल के माध्यम से NEFT, RTGS या IMPS द्वारा फंड ट्रांसफर कर सकते हैं।",
  ...
  mr: {
    ...
    balance:
      "आपल्या खात्याची शिल्लक तपासण्यासाठी, कृपया आपल्या DSOUZA BANK खात्यात लॉग इन करा.",
    loan: "आम्ही स्पर्धात्मक दरांवर गृहकर्ज, वैयक्तिक कर्ज आणि व्यवसाय कर्ज देतो.",
    transfer:
      "आपण आमच्या सुरक्षित ऑनलाइन बँकिंग पोर्टलद्वारे NEFT, RTGS किंवा IMPS द्वारे निधी हस्तांतरित करू शकता.",
  ...
  kn: {
    ...
    transfer:
      "ನೀವು ನಮ್ಮ ಸುರಕ್ಷಿತ ಆನ್‌ಲೈನ್ ಬ್ಯಾಂಕಿಂಗ್ ಪೋರ್ಟಲ್ ಮೂಲಕ NEFT, RTGS ಅಥವಾ IMPS ಮೂಲಕ ಹಣ ವರ್ಗಾಯಿಸಬಹುದು.",
  ...
  ta: {
    ...
    transfer:
      "நீங்கள் எங்கள் பாதுகாப்பான ஆன்லைன் வங்கி போர்டல் மூலம் NEFT, RTGS அல்லது IMPS மூலம் நிதி மாற்றலாம்.",
...
function getBotResponse(text: string, lang: ChatLang): string {
  ...
  if (
    ...
    lower.includes("ट्रांसफर") ||
    lower.includes("हस्तांतरण") ||
    lower.includes("ವರ್ಗಾವಣೆ") ||
    lower.includes("மாற்றம்")
  )
...
export default function ChatbotPanel({ isOpen, onClose }: ChatbotPanelProps) {
  ...
  useEffect(() => {
    const map: Record<string, ChatLang> = {
      en: "en",
      hi: "hi",
      mr: "mr",
      kn: "kn",
  ...
  // Initialize greeting when opened — intentionally only runs on isOpen change
  // biome-ignore lint/correctness/useExhaustiveDependencies: only run on open, not on every chatLang/messages change
  useEffect(() => {
  ...
  useEffect(() => {
    setMessages((prev) => {
      if (prev.length > 0 && prev[0].id === "greeting") {
        return [{ ...prev[0], text: GREETINGS[chatLang] }, ...prev.slice(1)];
      }
      return prev;
    });
  }, [chatLang]);

  // biome-ignore lint/correctness/useExhaustiveDependencies: messagesEndRef is a stable ref, scroll needed on message/typing change
  useEffect(() => {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-[#000000] leading-tight">
                DSOUZA AI
              </h2>
              <p className="text-[10px] text-[#000000] leading-tight">
                Banking Assistant
              </p>
            </div>
          </div>
        ...
        >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[#000000] text-xs font-medium hover:bg-white transition-colors"
              ...
              <div className="absolute top-9 left-0 bg-white border border-lavender-200 rounded-xl shadow-lg p-1 z-10 min-w-[120px]">
                {(Object.keys(LANG_LABELS) as ChatLang[]).map((code) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-lg text-xs text-[#000000] hover:bg-lavender-50 transition-colors ${chatLang === code ? "bg-indigo-50 font-semibold text-indigo-700" : ""}`}
                  >
                    {LANG_LABELS[code]}
                  </button>
                ))}
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${chatFontSize === "small" && !seniorMode ? "bg-indigo-100 text-indigo-700" : "text-[#000000] hover:bg-lavender-50"}`}
              title="Small font"
            >
              A
            </button>
            <button
              ...
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold transition-colors ${chatFontSize === "medium" && !seniorMode ? "bg-indigo-100 text-indigo-700" : "text-[#000000] hover:bg-lavender-50"}`}
              title="Medium font"
            >
              A
            </button>
            <button
              ...
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-base font-bold transition-colors ${chatFontSize === "large" && !seniorMode ? "bg-indigo-100 text-indigo-700" : "text-[#000000] hover:bg-lavender-50"}`}
              title="Large font"
            >
              A
            </button>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${seniorMode ? "bg-indigo-100 border-indigo-300 text-indigo-700" : "border-lavender-200 text-[#000000] hover:bg-lavender-50"}`}
          >
              ...
              <div
                className={`max-w-[80%] px-4 py-2.5 rounded-2xl ${FONT_SIZE_CLASS[effectiveFontSize]} ${
                  msg.role === "user"
                    ...
                    : "bg-white text-[#000000] rounded-tl-sm border"
                }`}
                style={
                  msg.role === "bot"
                    ? {
        ...
        >
                ...
                chatLang === "en"
                  ...
                  : chatLang === "hi"
                    ...
                    : chatLang === "mr"
                      ? "तुमचा संदेश लिहा..."
                      : chatLang === "kn"
                        ? "ನಿಮ್ಮ ಸಂದೇಶ ಟೈಪ್ ಮಾಡಿ..."
                        : "உங்கள் செய்தியை தட்டச்சு செய்யுங்கள்..."
              }
              className={`flex-1 px-4 py-2.5 rounded-xl border text-[#000000] bg-white placeholder:text-[#555555] focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all ${FONT_SIZE_CLASS[effectiveFontSize]}`}
              style={{ borderColor: "rgba(99,102,241,0.2)" }}
            />
            <button
              type="button"
          ...
          <p className="text-[10px] text-[#000000] mt-2 text-center">
