            ...
            {contactInfo.map((info, i) => (
              <MotionReveal key={info.title} delay={i * 80}>
                <div className="p-5 bg-white rounded-2xl border border-lavender-200/60 shadow-card hover:shadow-card-hover transition-all duration-300">
                  <div
                    className={`w-10 h-10 ${info.bg} rounded-xl flex items-center justify-center mb-3`}
                  >
                    <info.icon className={`w-5 h-5 ${info.color}`} />
                  </div>
                  <h3 className="font-bold text-sm text-[#000000] mb-2">
                    {info.title}
                  </h3>
                  {info.details.map((d) => (
                    <p
                      key={d}
                      className="text-xs text-[#1A1A1A] font-medium leading-relaxed"
                    >
                      {d}
                    </p>
                  ))}
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label
                      ...
                      className="block text-xs font-bold text-[#000000] mb-1.5"
                    >
                      Full Name *
                    </label>
                    <input
